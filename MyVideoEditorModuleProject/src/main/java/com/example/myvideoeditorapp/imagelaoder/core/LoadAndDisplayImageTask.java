...
import com.example.myvideoeditorapp.imagelaoder.core.decode.ImageDecodingInfo;
...
public final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
        ...
        if (!this.waitIfPaused()) {
            if (!this.delayIfNeed()) {
                ...
                try {
                    ...
                    this.checkTaskNotActual();
                    this.checkTaskInterrupted();
                } catch (TaskCancelledException var7) {
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                ...
                if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                    this.fireFailEvent(FailReason.FailType.DECODING_ERROR, (Throwable)null);
                }
            }
        } catch (IllegalStateException var4) {
            this.fireFailEvent(FailReason.FailType.NETWORK_DENIED, (Throwable)null);
        } catch (TaskCancelledException var5) {
            throw var5;
        } catch (IOException var6) {
            L.e(var6);
            this.fireFailEvent(FailReason.FailType.IO_ERROR, var6);
        } catch (OutOfMemoryError var7) {
            L.e(var7);
            this.fireFailEvent(FailReason.FailType.OUT_OF_MEMORY, var7);
        } catch (Throwable var8) {
            L.e(var8);
            this.fireFailEvent(FailReason.FailType.UNKNOWN, var8);
        }
    ...
    private Bitmap decodeImage(String imageUri) throws IOException {
        ...
        ImageDecodingInfo decodingInfo = new ImageDecodingInfo(this.memoryCacheKey, imageUri, this.uri, this.targetSize, viewScaleType,
                this.getDownloader(), this.options);
    ...
    private boolean downloadImage() throws IOException {
        ...
        if (is == null) {
            ...
            boolean var3;
            try {
                var3 = this.configuration.diskCache.save(this.uri, is, (var1, var2) -> false);
            } finally {
    ...
    private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
        ...
        if (targetFile != null && targetFile.exists()) {
            ...
            ImageDecodingInfo decodingInfo = new ImageDecodingInfo(this.memoryCacheKey,
                    ImageDownloader.Scheme.FILE.wrap(targetFile.getAbsolutePath()),
                    this.uri, targetImageSize, ViewScaleType.FIT_INSIDE, this.getDownloader(), specialOptions);
            ...
            Runnable r = new Runnable() {
                public void run() {
                    if (LoadAndDisplayImageTask.this.options.shouldShowImageOnFail()) {
                        LoadAndDisplayImageTask.this.imageAware.setImageDrawable(LoadAndDisplayImageTask.this.options
