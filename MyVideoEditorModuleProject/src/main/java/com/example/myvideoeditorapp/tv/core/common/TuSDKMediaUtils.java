...
package com.example.myvideoeditorapp.tv.core.common;
...
public class TuSDKMediaUtils
{
    ...
    public static int getColorRange(final String s) {
        final TuSDKVideoInfo videoInfo = getVideoInfo(s);
        if (videoInfo == null) {
            return -1;
        }
        TLog.e("[debug] color range %s", new Object[] { videoInfo.colorRange });
        return videoInfo.colorRange;
    }
    
    public static boolean checkIsMedia(final String s) {
        try {
            final TuSDKVideoInfo videoInfo = getVideoInfo(s);
            return videoInfo != null && videoInfo.width != 0 && videoInfo.height != 0 && videoInfo.fps != 0 && new TuSdkMediaDataSource(s).checkIsMediaFile();
        }
        catch (Exception ex) {
            return false;
        }
    ...
    public static TuSdkAudioInfo getAudioInfo(final TuSdkMediaDataSource tuSdkMediaDataSource) {
        final TuSdkAudioInfo tuSdkAudioInfo = new TuSdkAudioInfo(getAudioFormat(tuSdkMediaDataSource));
        String path = null;
        switch (tuSdkMediaDataSource.getMediaDataType().ordinal()) {
            case 1: {
                break;
            ...
            case 2: {
                path = tuSdkMediaDataSource.getPath();
                break;
            }
            default: {
        ...
        if (tuSdkMediaDataSource.isValid()) {
            TuSDKPostProcessJNI.readAudioInfo(path, tuSdkAudioInfo);
        }
        else {
            TuSDKPostProcessJNI.readAudioInfo(path, tuSdkAudioInfo);
        }
