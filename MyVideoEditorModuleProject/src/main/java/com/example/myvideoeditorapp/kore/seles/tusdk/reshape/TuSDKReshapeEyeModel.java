...
public class TuSDKReshapeEyeModel extends CosmeticModelBase
{
    ...
    public TuSDKReshapeEyeModel() {
        this.e = new float[TuSDKReshapeEyeModel.RESHAPE_MATERIAL_POINTS_LENGTH];
        this.f = new float[TuSDKReshapeEyeModel.RESHAPE_MATERIAL_POINTS_LENGTH * 3 / 2];
    ...
    public void setTextureCoordinate2(final FloatBuffer floatBuffer) {
        floatBuffer.put(TuSDKReshapeEyeModel.b);
    }
    
    public void setElementIndices(final IntBuffer intBuffer) {
        intBuffer.put(TuSDKReshapeEyeModel.a);
    }
    ...
    public void updateFace(final FaceAligment faceAligment, final TuSdkSize tuSdkSize) {
        ...
        for (int i = 0; i < TuSDKReshapeEyeModel.DEFAULT_VERTEX.length / 2; ++i) {
            this.e[2 * i] = TuSDKReshapeEyeModel.DEFAULT_VERTEX[2 * i];
            this.e[2 * i + 1] = TuSDKReshapeEyeModel.DEFAULT_VERTEX[2 * i + 1];
            this.f[3 * i] = TuSDKReshapeEyeModel.DEFAULT_TEXTURE[3 * i];
            this.f[3 * i + 1] = TuSDKReshapeEyeModel.DEFAULT_TEXTURE[3 * i + 1];
            this.f[3 * i + 2] = TuSDKReshapeEyeModel.DEFAULT_TEXTURE[3 * i + 2];
        }
        for (int j = 0; j < this.i.length; ++j) {
            final PointF normalize = PointCalc.normalize(this.i[j], tuSdkSize);
            this.e[TuSDKReshapeEyeModel.DEFAULT_VERTEX.length + 2 * j] = normalize.x * 2.0f - 1.0f;
            this.e[TuSDKReshapeEyeModel.DEFAULT_VERTEX.length + 2 * j + 1] = normalize.y * 2.0f - 1.0f;
            this.f[TuSDKReshapeEyeModel.DEFAULT_TEXTURE.length + 3 * j] = normalize.x;
            this.f[TuSDKReshapeEyeModel.DEFAULT_TEXTURE.length + 3 * j + 1] = normalize.y;
            this.f[TuSDKReshapeEyeModel.DEFAULT_TEXTURE.length + 3 * j + 2] = 1.0f;
        }
    }
    ...
    private void a(final FaceAligment faceAligment, final TuSdkSize tuSdkSize) {
        final float[] array = new float[TuSDKReshapeEyeModel.c.length * 2];
        for (int i = 0; i < TuSDKReshapeEyeModel.c.length; ++i) {
            this.g[i] = PointCalc.real(faceAligment.getEyeMarks()[TuSDKReshapeEyeModel.c[i]], tuSdkSize);
        ...
        final float[] array2 = new float[TuSDKReshapeEyeModel.c.length * 2];
        ...
        for (int j = 0; j < 16; ++j) {
            this.g[TuSDKReshapeEyeModel.c.length + j] = new PointF(array2[2 * j], array2[2 * j + 1]);
        }
        for (int k = 0; k < this.g.length; ++k) {
            this.i[k] = this.g[k];
        }
        final float[] array3 = new float[TuSDKReshapeEyeModel.c.length * 2];
        for (int l = 0; l < TuSDKReshapeEyeModel.d.length; ++l) {
            this.h[l] = PointCalc.real(faceAligment.getEyeMarks()[TuSDKReshapeEyeModel.d[l]], tuSdkSize);
        ...
        final float[] array4 = new float[TuSDKReshapeEyeModel.c.length * 2];
        ...
        for (int n11 = 0; n11 < 16; ++n11) {
            this.h[TuSDKReshapeEyeModel.c.length + n11] = new PointF(array4[2 * n11], array4[2 * n11 + 1]);
        }
    ...
    static {
        ...
        RESHAPE_TRIANGLES_MAP_LENGTH = TuSDKReshapeEyeModel.a.length;
        ...
        RESHAPE_MATERIAL_POINTS_LENGTH = TuSDKReshapeEyeModel.b.length;
