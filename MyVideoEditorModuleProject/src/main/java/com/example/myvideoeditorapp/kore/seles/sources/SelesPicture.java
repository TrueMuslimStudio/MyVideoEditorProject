...
public final class SelesPicture extends SelesOutput implements GLSurfaceView.Renderer
{
    ...
    public SelesPicture(final ByteBuffer byteBuffer, final int n, final int n2) {
        ...
        this.runOnDraw(new Runnable() {
            @Override
            public void run() {
                (SelesPicture.this.mOutputFramebuffer = SelesContext.sharedFramebufferCache()
                        .fetchFramebuffer(SelesFramebuffer.SelesFramebufferMode.TEXTURE_ACTIVE, SelesPicture.this.mInputTextureSize))
                        .bindTextureLuminance(byteBuffer, n, n2, SelesPicture.this.isShouldSmoothlyScaleOutput());
            }
        });
    }
