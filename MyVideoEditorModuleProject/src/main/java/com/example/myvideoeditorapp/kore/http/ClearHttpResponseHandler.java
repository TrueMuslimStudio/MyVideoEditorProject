...
public abstract class ClearHttpResponseHandler implements ResponseHandlerInterface
{
    ...
    public void sendResponseMessage(HttpResponse var1) throws IOException {
        if (!Thread.currentThread().isInterrupted()) {
            ...
            if (!Thread.currentThread().isInterrupted()) {
                if (var2 >= 300) {
                    this.sendFailureMessage(var2, var1.getAllHeaders(), var3, new HttpResponseException(var2, var1.getResponseMessage()));
                } else {
                    this.sendSuccessMessage(var2, var1.getAllHeaders(), var3);
                }
            }
        }

    }
    byte[] getResponseData(final HttpEntity httpEntity) throws IOException {
        byte[] byteArray = null;
        if (httpEntity != null) {
