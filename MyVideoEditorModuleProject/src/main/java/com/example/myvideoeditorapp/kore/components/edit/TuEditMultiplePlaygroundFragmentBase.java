...
import com.example.myvideoeditorapp.kore.network.analysis.ImageAutoColorAnalysis;
...
public abstract class TuEditMultiplePlaygroundFragmentBase extends TuResultFragment
{
    ...
    protected void handleStepPrevButton(final ImageView imageView, final DraftImageLoadListener draftImageLoadListener) {
        ...
        this.getProcessingDraftImageWrap().addBrushie(this.getProcessingDraftImageWrap().getHistories().remove(this.getProcessingDraftImageWrap().getHistories().size() - 1));
    ...
    protected void handleStepNextButton(final ImageView imageView, final DraftImageLoadListener draftImageLoadListener) {
        ...
        final List<File> histories = this.getProcessingDraftImageWrap().getHistories();
        final File file = brushies.remove(brushies.size() - 1);
        if (!histories.contains(file)) {
            histories.add(file);
        }
        this.refreshStepStates();
        this.asyncLoadImage(imageView, this.getProcessingDraftImageWrap(), false, draftImageLoadListener);
    ...
    @Override
    protected void saveToAlbum(final TuSdkResult tuSdkResult) throws IOException {
